...
if(isset($_POST['btn3'])){
    ...
    $select_product_name1 = "SELECT VEN_NAME FROM `venue` WHERE VEN_NAME = '$name1'" or die('query failed');
    $res = mysqli_query($conn,$select_product_name1);
    if(mysqli_num_rows($res) > 0){
        echo "<script>alert('Venue already added')</script>";
    }else{
       $add_product_query1 = "INSERT INTO `venue`(VEN_NAME, DESCRIPTION, IMAGE) VALUES('$name1','$desc1', '$image1')" or die('query failed');
       ...
       if($add_product_query1){
          if($image_size > 20000000){
            ...
            echo "<script>alert('Venue added Sucessfully')</script>";
          }
       }else{
          echo "<script>alert('Venue could not be added!')</script>";
       }
    }
 }
