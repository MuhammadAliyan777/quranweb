...
if(isset($_POST['btn4'])){
    ...
    $select_product_name1 = "SELECT EVE_NAME FROM `event` WHERE EVE_NAME = '$name1'" or die('query failed');
    $res = mysqli_query($conn,$select_product_name1);
    if(mysqli_num_rows($res) > 0){
        echo "<script>alert('Event already added')</script>";
    }else{
       $add_product_query1 = "INSERT INTO `event`(EVE_NAME, DESCRIPTION, IMAGE) VALUES('$name1','$desc1', '$image1')" or die('query failed');
       ...
       if($add_product_query1){
          ...
          echo "<script>alert('Event could not be added!')</script>";
       }
    }
 }
